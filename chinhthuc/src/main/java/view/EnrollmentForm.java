 ...
 * @author
 */
public class EnrollmentForm extends javax.swing.JInternalFrame {
    ...
    AccountDAO dao = null;
    EnrollmentService enrollment_dao;
    private ArrayList<Enrollment> list;

    public EnrollmentForm() throws SQLException {
        initComponents();
        dao = new AccountDAO();
        enrollment_dao = new EnrollmentService();
        list = enrollment_dao.selectAll();
    ...
    public final void initTable() {
        ...
        String[] headerTbl = new String[]{"Id", "Nguời đăng kí", "Khóa học", "Ngày đăng kí", "Ngày kết thúc", "Trạng thái"};
        tblModel.setColumnIdentifiers(headerTbl);
        tblAccount.setModel(tblModel);
    }

    public void loadDataToTable(ArrayList<Enrollment> acc) {
        try {

            tblModel.setRowCount(0);
            SimpleDateFormat dateFormat = new SimpleDateFormat("yyyy-MM-dd");
            for (Enrollment i : acc) {
                //ep kieu di
                Date start = dateFormat.parse(i.getStart_date());
                Date finish = dateFormat.parse(i.getFinish_date());

                if (start.before(new Date()) && finish.after(new Date())) {
                    i.setStatus(1);
                } else {
                    i.setStatus(0);
                }
                tblModel.addRow(new Object[]{
                    i.getId(), i.getUser().getUserName(), i.getCourse().getName(), i.getStart_date(), i.getFinish_date(), i.isStatus() == 1 ? "Đang hoạt động" : "Đã kết thúc"
                });
            }
        } catch (Exception e) {
        }
    ...
    public Enrollment getEnrollmentSelect() throws SQLException {
        int i_row = tblAccount.getSelectedRow();

        Enrollment acc = enrollment_dao.selectById((int) ( tblAccount.getValueAt(i_row, 0)));
        return acc;
    }
    ...
    private void initComponents() {
        ...
        cbxLuachon.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Tất cả", "Tên tài khoản", "Email", "Vai trò" }));
    ...
    private void btnAddActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnAddActionPerformed
        // TODO add your handling code here:
        try{
            AddEnro a = new AddEnro(this, (JFrame) javax.swing.SwingUtilities.getWindowAncestor(this), rootPaneCheckingEnabled);
            a.setVisible(true);
        }catch(Exception e){
            e.printStackTrace();
        }
    ...
    private void btnDeleteAccountActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnDeleteAccountActionPerformed
        ...
        if (tblAccount.getSelectedRow() == -1) {
            ...
            Enrollment select = null;
            try {
                select = getEnrollmentSelect();
            } catch (SQLException ex) {
                Logger.getLogger(EnrollmentForm.class.getName()).log(Level.SEVERE, null, ex);
            }
                int checkVl = JOptionPane.showConfirmDialog(this, "Bạn có chắc chắn muốn xóa đối tượn này ?", "Xác nhận xóa", JOptionPane.YES_NO_OPTION);
                if (checkVl == JOptionPane.YES_OPTION) {
                    try {
                        enrollment_dao.delete(select);
                        JOptionPane.showMessageDialog(this, "Xoá thành công !");
                        loadDataToTable(enrollment_dao.selectAll());
                    } catch (Exception e) {
        ...
        try{
            ...
            switch (luachon) {
                ...
                case "Tên tài khoản":
                    ...
                    break;
                case "Email":
                    result = SearchAccount.getInstance().searchEmail(searchContent);
                    break;
                case "Vai trò":
